...
**Decision:** The chat panel is visible on ALL tabs (not just commentable ones) and includes a session selector dropdown (`<select class="session-select">`) in its header. The dropdown lists active sessions as `{cwd} ({session_id_short}) — il y a X min`. Users can connect any tab to any active session. The chat input is disabled when no session is selected. The chat panel (`.chat-panel`) is a **flex child** of `#main` (not `position: absolute`, not inside `#content-scroll`). It sits below `#content-scroll` in the flex column layout naturally. The `.chat-resizer` is a **separate DOM element** — a flex sibling above `.chat-panel` in `#main` — not inside the chat panel. It's 8px tall with a 40px centered bar indicator that turns blue on hover, and adjusts `height` (not `max-height`) of `.chat-messages` (60px–500px), persisted in localStorage. The comment form and chat input both include a submit button (➤) alongside Enter-to-send. Content action buttons (refresh ⟳, width toggle ⬌/⬄) are inside `#content-scroll` with `position: sticky; bottom: 12px; float: right`, grouped in a small bar with shared background/border. The sidebar toggle (☰) is the first element in `#tab-bar` (left of tabs), not in the sidebar header — a single button toggles sidebar visibility. The sidebar has `border-right` for visible separation in light mode. The comment button/form dismiss on scroll of `#content-scroll`.

**Rationale:** Making the chat panel visible on all tabs allows users to interact with Claude from any context, not just from tabs opened via MCP. The session selector gives users explicit control over which Claude Code session receives their messages, which is clearer than implicit session-per-tab routing. Disabling input when no session is selected prevents confusion. The submit button improves discoverability — not all users know they can press Enter to send. Using flex layout instead of absolute positioning ensures the chat panel participates in the natural document flow and doesn't overlap content. A separate resizer element avoids nesting complexity and makes the grab zone a clear flex sibling. Sticky action buttons inside the scroll area keep them accessible without occluding the chat panel. Dismissing comment UI on scroll prevents stale viewport-positioned elements from lingering.
