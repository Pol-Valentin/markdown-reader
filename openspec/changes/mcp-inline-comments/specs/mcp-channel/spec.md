...
### Requirement: Channel reconnects on close without relaunching
When the persistent subscriber connection drops, the channel SHALL call `tryReconnectWithoutRelaunch()`. This function polls for the Reader's socket every 5 seconds indefinitely, without launching a new Reader instance. Once the Reader becomes available again, the channel reconnects and re-subscribes. There is no maximum attempt limit.

#### Scenario: Reader exits while channel is running
- **WHEN** the Reader process exits and the subscriber connection closes
- **THEN** the channel calls `tryReconnectWithoutRelaunch()`
- **THEN** it polls every 5 seconds for the Reader's socket

#### Scenario: Reader is restarted by the user
- **WHEN** the Reader is restarted and the socket becomes available
- **THEN** the channel detects the socket via ping, connects, and re-subscribes
- **THEN** the channel is fully operational again without requiring an `open_file` call
...
### Requirement: Channel tries to connect at startup
The channel SHALL call `tryReconnectWithoutRelaunch()` at startup, immediately after MCP initialization. If the Reader is already running, the channel connects and subscribes right away. If the Reader is not running, the channel polls every 5 seconds indefinitely until it becomes available. The channel does NOT launch the Reader at startup — it only connects to an already-running instance.

#### Scenario: Reader is already running at channel startup
- **WHEN** the channel process starts and the Reader is already running
- **THEN** the channel connects to the socket and subscribes immediately

#### Scenario: Reader is not running at channel startup
- **WHEN** the channel process starts and no Reader is running
- **THEN** the channel polls every 5 seconds for the Reader's socket
- **THEN** once the Reader is launched (by user or by an `open_file` call), the channel connects and subscribes
