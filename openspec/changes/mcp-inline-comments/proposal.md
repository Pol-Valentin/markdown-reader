...
- `inline-comments-ui`: Frontend comment UI — text selection commenting, Mermaid/code block commenting, chat panel, comment payload construction
...
- **Modified Frontend:** `renderer.js` (`data-source` attributes on blocks), `main.js` (wire comments module, listen for replies), `styles.css` (comment UI + chat panel + chat input area styles, font smoothing, `position: fixed` for comment elements)
